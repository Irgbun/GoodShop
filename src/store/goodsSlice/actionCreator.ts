...
export const fetchGoods = ({ id, type, text }: GetDataGoods) => async(dispatch: Dispatch<any>) => {
    dispatch(getGoods())
    new Api().getDataGoods({ id, type, text }).then((data) => dispatch(getGoodsSuccess(data.items))).catch(() => dispatch(getGoodsFailure()))
}